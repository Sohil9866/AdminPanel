...
const Table = ({ columns, data, actions, type, nameId }) => {
  const {
  ...
  const getPath = (type, id) => {
    console.log(id);
    switch(type){
      case "employee": return `/employees/edit-employee/${id}`;
      default: return ("/");
    }
  }
                  ...
                  >
                    {type === "restaurant" && column.accessor === "Name" ? (
                      <div className="flex items-center">
                        <img
                          src={`https://admin.kutumbabazar.com/foodapi/${row.Images[0].Url}`}
                      ...
                      <Link to={`/orders/orderdetails/${row["Id"]}`}>
                        <span className="block text-[#5C59E8]">
                          {row["Id"]}
                        </span>
                      </Link>
                    ) : nameId && column.accessor === "Name" ? (
                      <>
                        <span className="text-[#333843]">{row["Name"]}</span>
